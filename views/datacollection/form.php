...
function savefield(val,id) {
	...
	$.post("/?control=datacollection/main/savefield&ajax",{"field":id,"value":val,"control":"<?=$_GET['id']?>","lasttab":active},
...
function finishprocess() {
	...
	if(!$("#declaration").is(":checked")) {
		...
		$.post("/?control=datacollection/main/updatefinish&ajax",{"control":"<?=$_GET['id']?>"},
			function html(response) {
...
$(function() {
	$(".checkboxes-segments input[type=checkbox]").click(function() {
		...
		$.post("/?control=datacollection/main/savesegments&ajax",{"value":segments,"control":<?=$_GET['id']?>},
			function html(c) {
	...
	$(".checkboxes-industries input[type=checkbox]").click(function() {
		...
		$.post("/?control=datacollection/main/saveindustries&ajax",{"value":industries,"control":<?=$_GET['id']?>},
			function html(c) {
	...
	$(".checkboxes-services input[type=checkbox]").click(function() {
		...
		$.post("/?control=datacollection/main/saveservices&ajax",{"value":services,"control":<?=$_GET['id']?>},
			function html(c) {
...
function saveshareholders(save) {
	...
	$.post("/?control=datacollection/main/saveshareholders&ajax",{"control":<?=$_GET['id']?>,"shareholders":lines},function html(c) {
...
<div id="selection-container">
	...
	<select name="vendorcontrol" id="vendorcontrol" onchange="goto(this.value);">
		<?php if (!$_GET['id']) echo "<option value=\"\">- Select a vendor -</option>" ?>
		<?php
		foreach($vendorslist as $vendoritem) {
			if($vendoritem->vendorcontrol == $_GET['id']) {
				?>
...
<?php if($_GET['id']."" == "") { ?>
	<div class="datacollect-welcome">
		<p><a href="/assets/images/Central Suppliers Communication Letter.pdf" style="text-decoration:underline; "><strong>Download Official PremierVendor Communication.</strong></a></p>
		<p>Premier Foods Trading have embarked on a project to ensure that your vendor data currently in the Premier Foods Trading database is updated, verified and maintained by you, the vendor.</p>
		<p>It is imperative that you update and maintain this data regularly to ensure more effective communication of business transactions with Premier Foods Trading.</p>
		<p>The completion date for the collection of all vendor data is <strong>28th February 2014</strong>.</p>

		<p>Please note this web page is supported by the following browsers : Internet Explorer 8 and above, Mozilla firefox and Google Chrome. Here is a list of places to download these browsers : </p>
		<ul>
			...
			<li><a href="http://windows.microsoft.com/en-us/internet-explorer/download-ie">Download Internet Explorer</a></li>
		</ul>
	</div>
...
<?php if($_GET['id']!= "") { ?>
	...
	<div id="step2" style="position:relative;">
		<table class="form-table">
			<tr>
				<td colspan="3"><h2>General Details</h2></td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="3">
					...
					<div id="add-container">
						<input type="button" onclick="addanother();" value="Add more..." />
					</div>
				</td>
			</tr>
			<tr>
				<td>
					<input type="button" onclick="validatestep(2);" value="Continue To Step 3" />
				</td>
			</tr>
		</table>
	</div>
	...
	<tr>
		<td colspan="3"><h2>Sales Contact</h2></td>
	</tr>
	<tr>
		<td class="form-table-label">Title</td>
		<td class="form-table-data">
	...
	<tr>
		<td colspan="3"><h2>Finance Contact</h2></td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="3">
			<input type="button" onclick="buttonback(1);" value="Back To Step 2" /> <input type="button" onclick="validatestep(3);" value="Continue To Step 4" />
		</td>
	</tr>
	</table>
	</div>
	...
	<tr>
		<td colspan="3"><h2>Which Industries Do You Serve?</h2></td>
	...
	<tr>
		<td colspan="3"><h2>Which market segments do you belong to?</h2></td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="3">
			<h2>What products or services do you offer?</h2>
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="3">
			<input type="button" onclick="buttonback(2);" value="Back To Step 3" /> <input type="button" onclick="validatestep(4);" value="Continue To Step 5" />
		</td>
	</tr>
	</table>
	</div>
	<div id="step5">
		<table class="form-table">
			...
			<tr>
				<td colspan="3">
					<input type="button" onclick="buttonback(3);" value="Back To Step 4" /> <input type="button" onclick="validatestep(5);" value="Continue To Step 6" />
				</td>
			</tr>
		</table>
	</div>
	<div id="step6">
		<table class="form-table">
			<form method="post" action="/?control=datacollection/main/upload&ajax&vendorcontrol=<?=$_GET['id']?>"  target="my-iframe" enctype="multipart/form-data">
				<tr>
				...
				<tr>
					<td colspan="2">
						<iframe name="my-iframe" id="my-iframe" frameborder="0" width="100%" height="200" src="/?control=datacollection/main/upload&ajax&vendorcontrol=<?=$_GET['id']?>"></iframe>
					</td>
				</tr>
				<tr>
					<td colspan="2">
				...
				<tr>
					<td colspan="3">
						<input type="button" onclick="buttonback(4);" value="Back To Step 5" /> <input type="button" value="Complete Process" onclick="finishprocess();" />
					</td>
				</tr>
			</form>
		</table>
